...
class WireNet:
	...
	def __init__(self, n, l, sdl, d, sk, wres, ires, rsh, debug=False, xsort=True):
		...
		self.angleskew = sk            # Skew of anglular distribution of wires // to x-direction (kappa in von Mises distribution)
	...
	def parameters(self):
		...
		print "Number of wires: %i"%self.n
		print "Wire lengths: %f with standard deviation: %f"%(self.lav, self.lstd)
		print "Sample size: %f"%self.sample_dimension
		print "Wire areal density: %f"%self.wire_density
		print "Angular dispersion: %f (calculated S-parameter of %f)"%(self.angleskew, self.s)
		print "Wire resistance per length: %f"%self.wire_res
		print "Wire interconnect resistance: %f"%self.intersect_res
		print "Matrix sheet resistance: %f"%self.sheet_res
		return
		...
		for j in range(2*self.n):
			...
			if wireindex < self.n:
				...
				for slwireindex in searchlist:
					...
					if not noprint:
						print "Checking wires: %3i and %3i"%(wireindex, slwireindex),
	...
	def solve(self, fulloutput=False):
		...
		if fulloutput:
			return val, vec, c_ij, lmatrix, intersectionlist_array
		else:
			return val, vec
