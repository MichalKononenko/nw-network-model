...
nwdensity = np.array([0.1, 0.020, 0.015, 0.010, 0.007, 0.005])   # Nanowires per sq micron
nwnumber = (nwdensity*substratesize**2).astype(int)             # Number of nanowires
nwinterres = np.array([10.0, 100.0, 1000.0])                    # Resistance between wires
...
if __name__ == "__main__":
	...
	# Loops for different starting parameters
	for l1, nwn in enumerate(nwnumber):
		for l2, nwi in enumerate(nwinterres):
			nets = []
			result = []
			runnumber = np.random.randint(10000)
			procs = 2                      # 6 processes running for multiprocessing (one for each nanowire network)
			for i in xrange(procs):
				# Create nanowire network class
				n = WireNet(nwn, nwlength, nwlength_sd, substratesize, nwanglekappa, nwresistance, nwi, matrixrsheet)
			...
			for i, n in enumerate(nets):
				...
				print filename
				eigenvalues, eigenvectors, conductance_matrix, laplacian_matrix, list_of_nodes = result[i]
				for p in testpoints:
				...
				#mf.dataOutputGen(filename + "_VEC.txt", path, n.eigenvectors)
